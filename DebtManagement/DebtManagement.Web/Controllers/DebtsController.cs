/* the problem with this one is everyone can see all debts, i want to see only my debts
using AutoMapper;
...
/* this one works, but i also want to print ClientId,(or) also dont forget to remove ClientId from add and edit?. auto fill it with the current ClientId. debts etc shouldnt be visible before login*/
using AutoMapper; 
using DebtManagement.Web.DTOs;
using DebtManagement.Web.Services;
...
namespace DebtManagement.Web.Controllers
{
    public class DebtsController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            // Get the current user's ClientId
            ...
            DebtsViewModel viewModel = new DebtsViewModel
            {
                CreditCardDebts = _mapper.Map<List<DebtDTO>>(await _debtService.GetDebtsByClientIdAndTypeAsync(clientId, DebtType.CreditCard)),
                LoanDebts = _mapper.Map<List<DebtDTO>>(await _debtService.GetDebtsByClientIdAndTypeAsync(clientId, DebtType.Loan)),
                AvansDebts = _mapper.Map<List<DebtDTO>>(await _debtService.GetDebtsByClientIdAndTypeAsync(clientId, DebtType.Avans)),
                MonthlyRentsDebts = _mapper.Map<List<DebtDTO>>(await _debtService.GetDebtsByClientIdAndTypeAsync(clientId, DebtType.MonthlyRents)),
                OtherDebts = _mapper.Map<List<DebtDTO>>(await _debtService.GetDebtsByClientIdAndTypeAsync(clientId, DebtType.Other))
            };
        ...
        public async Task<IActionResult> Create(DebtDTO debtDto)
        {
            if (ModelState.IsValid)
            {
                var debt = _mapper.Map<Debt>(debtDto);
                var user = await _userManager.GetUserAsync(User);
                debt.ClientId = user.ClientId; // set the ClientId to the current user's ID
                await _debtService.AddDebtAsync(debt);
                return RedirectToAction(nameof(Index));
            }
        ...
        public async Task<IActionResult> Edit(int id, DebtDTO debtDto)
        {
            ...
            if (ModelState.IsValid)
            {
                var debt = _mapper.Map<Debt>(debtDto);
                var user = await _userManager.GetUserAsync(User);
                debt.ClientId = user.ClientId; // ensure the ClientId is the current users Id
                await _debtService.UpdateDebtAsync(debt);
                return RedirectToAction(nameof(Index));
            }
